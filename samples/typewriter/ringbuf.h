...
struct ringbuf_t {
    ...
    pthread_cond_t
        cond_full; /* producer needs to wait when the buffer is full */
    pthread_cond_t
...
/* insert (i.e., append) a character into the buffer; if the buffer is full, the
 * caller thread will be blocked */
extern void rb_insert(struct ringbuf_t *rb, int c);

/* retrieve a character at the front of the ring buffer; if the buffer is empty,
