...
struct mythread_t {
    ...
    int P;        /* number of threads in one dimension; there are PxP number of
                     threads */
...
void block_matmul(int crow, int ccol, /* corner of C block */
    ...
    for (i = 0; i < n; i++)
        for (j = 0; j < n; j++)
            for (k = 0; k < n; k++)
                c[crow + i][ccol + j] +=
                    a[arow + i][acol + k] * b[brow + k][bcol + j];
}
...
void *work(void *arg) {
    ...
    int n = t->N / t->P, i;
    for (i = 0; i < t->P; i++) {
        block_matmul(n * t->x, n * t->y, n * t->x, n * i, n * i, n * t->y, n,
                     t->N, t->a, t->b, t->c);
    }
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    int p = (int)sqrt(pp);
    if (p * p != pp) {
        printf("ERROR: #threads not square number!\n");
        return 3;
    }
    ...
    if (n / p * p != n) {
        printf("ERROR: matrix dimension cannot be divided by p.\n");
        return 7;
    }
