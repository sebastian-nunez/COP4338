// Define a function AdjustState() that takes one integer parameter as the air temperature and one character parameter passed by pointer as the thermostat's state ('L' means low, 'S' means stable, and 'H' means high). The function updates the state with:

// 'L' if the temperature is less than 60.
// 'H' if the temperature is greater than 60.
// The function returns true if the state has been updated, and returns false otherwise.
